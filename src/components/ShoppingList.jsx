export default function ShoppingList({ items, onAdd, onSub, onRemove, onClear }) {
  const total = items.reduce((acc, it) => acc + it.plant.precio * it.qty, 0)

  return (
    <div className="flex flex-col h-full bg-white">
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 bg-emerald-700 text-white">
        <h2 className="font-bold text-lg">Lista de compra</h2>
        {items.length > 0 && (
          <button
            onClick={onClear}
          ...
          items.map((it) => (
            <div key={it.plant.id} className="flex flex-col rounded-xl ring-1 ring-gray-200 p-3 gap-2">
              <div className="flex items-start justify-between gap-2">
                <div>
                  <div className="font-semibold text-gray-900 text-[15px]">{it.plant.nombre}</div>
                  <div className="text-xs text-gray-500">
                    {(it.plant.precio * it.qty).toLocaleString('es-PY')} Gs
                  </div>
                </div>
                <button
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="text-sm">
                  ...
                  <span className="font-semibold text-gray-800">
                    {it.plant.precio.toLocaleString('es-PY')} Gs
                  </span>
                </div>
              </div>
            </div>
      ...
      <div className="border-t border-gray-200 px-4 py-3 flex items-center justify-between bg-gray-50">
        <div>
          ...
          <div className="font-bold text-xl text-emerald-700">
            {total.toLocaleString('es-PY')} <span className="text-sm">Gs</span>
          </div>
        </div>
        <button
          className="bg-emerald-600 text-white font-bold px-5 py-3 rounded-full active:scale-95 transition shadow"
        >
          Continuar →
        </button>
      </div>
